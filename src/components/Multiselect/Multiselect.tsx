...
const CustomMultiselectButtonContainer = styled.div`
  position: relative;
  display: flex;
  align-items: center;
`;

const CustomMultiselectButton = styled.button<{
  disabled?: boolean;
  error?: boolean;
  warning?: boolean;
  hasIcon?: boolean;
...
const CustomMultiselectMenu = styled.div<{ isOpen: boolean }>`
  position: absolute;
  top: 100%;
  left: 0;
  right: 0;
...
export const Multiselect: React.FC<MultiselectProps> = ({
  ...
  const renderTags = () => {
    if (selectedOptions.length === 0) {
      return <PlaceholderText isEmpty>{placeholder}</PlaceholderText>;
    }
  ...
  return (
    <MultiselectContainer className={className}>
      ...
      <CustomMultiselectContainer ref={multiselectRef}>
        <CustomMultiselectButtonContainer>
        ...
        <CustomMultiselectMenu isOpen={isOpen}>
          <ScrollArea>
            <OptionsList>
